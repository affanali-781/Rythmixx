(function( PKAE ) {
	...
	try {
		...
		if (!window.supabase) {
			...
			console.log('Supabase client initialized in actions.js');
		} else {
...
// Modified upload function to accept metadata
function uploadToSupabase(file, fileName, metadata) {
	...
	if (!metadata.userId) {
		console.error('Upload failed: No user ID provided in metadata:', metadata);
		OneUp('Error: No user ID available', 3000);
		return;
	}

	// Access the Supabase storage API
	const bucket = supabase.storage.from('songsbucket');
	
	console.log('Attempting upload to bucket:', {
		bucketName: 'songsbucket',
		fileName: fileName,
		uploadOptions: {
			cacheControl: '3600',
			upsert: false
	...
	bucket
	  ...
	  .then((response) => {
		console.log('Storage upload response:', response);

		if (response.error) {
		  console.error('Error uploading file to Supabase:', response.error);
		  OneUp('Error: ' + response.error.message, 3000);
		} else {
		  // Get the public URL
		  const publicUrl = bucket.getPublicUrl(fileName).publicURL;
		  ...
		  console.log('Inserting song data into database:', songData);
		  ...
		  supabase
			...
			.insert([songData])
			.then((dbResponse) => {
			  console.log('Database insert response:', dbResponse);
			  if (dbResponse.error) {
				console.error('Error inserting metadata:', dbResponse.error);
				OneUp('Error saving metadata: ' + dbResponse.error.message, 3000);
			  } else {
				console.log('Upload complete. Final data:', {
	  ...
	  .catch((error) => {
		...
		OneUp('Error uploading: ' + (error.message || 'Unknown error'), 3000);
	  });
}
  
// Function to insert metadata into the database
function insertMetadataToDatabase(songData) {
	...
	supabase
	  ...
	  .then((response) => {
		if (response.error) {
		  console.error('Error inserting metadata into database:', response.error.message);
		  OneUp('Error saving metadata: ' + response.error.message, 3000);
		} else {
		  console.log('Metadata inserted successfully:', response.data);
		  OneUp('Song metadata saved successfully!', 2000);
		}
	  })
	  .catch((error) => {
		console.error('Error inserting metadata:', error);
		OneUp('Error saving metadata: ' + (error.message || 'Unknown error'), 3000);
	  });
}
...
////////////////////////////////////////////////////////////////
		...
		this.uploadToSupabase=uploadToSupabase; 
	...
	window.addEventListener('message', function(event) {
	  ...
	  if (event.data.type === 'INIT_EDITOR') {
	    console.log('Initializing editor with user ID:', event.data.userId);
