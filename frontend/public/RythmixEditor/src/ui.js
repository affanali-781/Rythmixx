(function ( w, d, PKAE ) {
	...
	// Global state to store metadata
	...
	// Create a global object to store editor state
	window.PKEditorState = {
		currentUserId: null,
		setUserId: function(userId) {
			console.log('Setting editor user ID:', userId);
			this.currentUserId = userId;
			// Trigger any necessary UI updates
			if (window.PKAudioEditor && window.PKAudioEditor.ui) {
				const uploadButton = document.querySelector('[data-id="upload"]');
				if (uploadButton) {
					uploadButton.classList.remove('pk_inact');
				}
			}
		}
	};
	...
	window.addEventListener('message', function(event) {
		...
		if (event.data.type === 'SET_USER_ID') {
			...
			window.PKEditorState.setUserId(userId);
		}

		if (event.data.type === 'INIT_EDITOR') {
			console.log('Received INIT_EDITOR message with data:', event.data);
			
			// Store metadata
			window.editorMetadata = {
				userId: event.data.userId,
				trackId: event.data.trackId,
				title: event.data.trackName,
				artist: event.data.trackArtist,
				genres: event.data.trackGenres,
				projectId: event.data.projectId,
				img: event.data.coverImage
			};
			
			// Also set currentUserId for compatibility
			window.currentUserId = event.data.userId;
			window.PKEditorState.setUserId(event.data.userId);
		}

		if (event.data.type === 'UPDATE_METADATA') {
			console.log('Updating editor metadata:', event.data.metadata);
			
			// Store the metadata
			window.editorMetadata = event.data.metadata;
			window.currentUserId = event.data.metadata.userId; // For compatibility with existing code
			
			console.log('Editor metadata updated:', window.editorMetadata);
			
			// Update the form fields if they exist
			const updateFormFields = () => {
				const form = document.querySelector('.pk_modal form');
				if (form) {
					const metadata = window.editorMetadata;
					if (metadata.title) form.querySelector('#supabase-title').value = metadata.title;
					if (metadata.artist) form.querySelector('#supabase-artist').value = metadata.artist;
					if (metadata.genres) form.querySelector('#supabase-genre').value = metadata.genres.join(', ');
					if (metadata.img) form.querySelector('#supabase-img').value = metadata.img;
					if (metadata.projectId) form.querySelector('#supabase-project').value = metadata.projectId;
				}
			};
			
			// Try to update form if it's open
			updateFormFields();
		}
	}, false);

	// Supabase client is now initialized in actions.js
	console.log('Using Supabase client from actions.js');

	// Simplified check function that uses the global instance from actions.js
	...
	async function validateBucketAccess(bucketName) {
		try {
			const { data, error } = await window.supabase.storage.from(bucketName).list();
			if (error) {
			...
			console.error(`Error validating ${bucketName} access:`, error);
			return false;
		}
	}
				...
				children : [
...

					{
						...
						action: function() {
							...
							// Use the globally initialized Supabase instance
							if (!window.supabase) {
								console.error('Supabase client not initialized');
								OneUp('Error: Upload service not ready', 3000);
								return;
							}

							if (!window.currentUserId) {
								console.error('No user ID available');
								OneUp('Error: User ID not available. Please log in again.', 3000);
								return;
							}
							...
							new PKSimpleModal({
								...
								ondestroy: function (q) {
									app.ui.InteractionHandler.on = false;
									app.ui.KeyHandler.removeCallback('modalTempSupabase');
								},
								buttons: [
									{
										title: 'Upload',
										clss: 'pk_modal_a_accpt',
										callback: async function(q) {
											try {
												// Check Supabase initialization
												const supabaseOk = await checkSupabase();
												if (!supabaseOk) {
													OneUp('Error: Upload service not ready. Please try again later.', 3000);
													return;
												}

												// Validate bucket access
												const [songsBucketOk, imagesBucketOk] = await Promise.all([
													validateBucketAccess('songsbucket'),
													validateBucketAccess('cover-images')
												]);

												if (!songsBucketOk || !imagesBucketOk) {
													OneUp('Error: Storage buckets not accessible. Please try again later.', 3000);
													return;
												}

												// Get form values
												const fileName = q.el_body.querySelector('#supabase-filename').value.trim();
												const title = q.el_body.querySelector('#supabase-title').value.trim();
												const artist = q.el_body.querySelector('#supabase-artist').value.trim().split(',').map(a => a.trim());
												const genre = q.el_body.querySelector('#supabase-genre').value.trim().split(',').map(g => g.trim());
												const imgFile = q.el_body.querySelector('#supabase-img-file').files[0];
												const imgUrl = q.el_body.querySelector('#supabase-img-url').value.trim();
												const projectId = q.el_body.querySelector('#supabase-project').value.trim();

												// Get selected region if any
												const region = app.engine.wavesurfer.regions.list[0];
												const export_sel = region ? [region.start, region.end] : false;

												// First get the audio buffer
												app.engine.GetAudioBuffer(export_sel, async function(audioBlob) {
													try {
														...
														const filePath = `songs/${window.currentUserId}/${uniqueFileName}`;
														...
														if (imgFile) {
															try {
																...
																const imgPath = `covers/${window.currentUserId}/${timestamp}.${imgExt}`;

																// Upload image to cover-images bucket
																const { data: imgData, error: imgError } = await window.supabase.storage
																	.from('cover-images')
																...
																const { data: imgUrlData } = window.supabase.storage
																	.from('cover-images')
																	.getPublicUrl(imgPath);

																finalImgUrl = imgUrlData.publicUrl;
																console.log('Image uploaded successfully:', finalImgUrl);
															} catch (imgError) {
																console.error('Failed to upload image:', imgError);
																// Continue with song creation even if image upload fails
															}
														}
														...
														OneUp('Upload successful!', 2000);
														q.Destroy();

													} catch (error) {
														console.error('Upload process failed:', error);
														OneUp('Error: ' + error.message, 3000);
													}
												});
											} catch (error) {
								...
								body: `
									<div class="pk_row">
										<label for="supabase-filename">File Name*</label>
										<input style="min-width:250px" placeholder="filename.mp3" value="audiomass-upload.mp3"
										class="pk_txt" type="text" id="supabase-filename" required />
									</div>
									<div class="pk_row">
										<label for="supabase-title">Title</label>
										<input style="min-width:250px" placeholder="Song title"
										class="pk_txt" type="text" id="supabase-title" value="${window.editorMetadata.title || ''}" />
									</div>
									<div class="pk_row">
										<label for="supabase-artist">Artist(s)</label>
										<input style="min-width:250px" placeholder="Artist names (comma-separated)"
										class="pk_txt" type="text" id="supabase-artist" value="${Array.isArray(window.editorMetadata.artist) ? window.editorMetadata.artist.join(', ') : window.editorMetadata.artist || ''}" />
									</div>
									<div class="pk_row">
										<label for="supabase-genre">Genre(s)</label>
										<input style="min-width:250px" placeholder="Genres (comma-separated)"
										class="pk_txt" type="text" id="supabase-genre" value="${Array.isArray(window.editorMetadata.genres) ? window.editorMetadata.genres.join(', ') : window.editorMetadata.genres || ''}" />
									</div>
									<div class="pk_row">
										<label for="supabase-img-file">Cover Image</label>
										<input style="min-width:250px" type="file" accept="image/*" id="supabase-img-file" />
										<div style="margin-top: 5px;">
											<small>Or use URL:</small>
											<input style="min-width:250px" placeholder="Cover image URL"
											class="pk_txt" type="text" id="supabase-img-url" value="${window.editorMetadata.img || ''}" />
										</div>
									</div>
									<div class="pk_row">
										<label for="supabase-project">Project ID</label>
										<input style="min-width:250px" placeholder="Project ID"
										class="pk_txt" type="text" id="supabase-project" value="${window.editorMetadata.projectId || ''}" />
									</div>
									<div class="pk_row">
										<input type="radio" class="pk_check" id="up1" name="uploadsel" checked value="whole">
										<label for="up1">Upload whole file</label>
										<input type="radio" class="pk_check" id="up2" name="uploadsel" value="sel">
										<label for="up2">Upload selection only</label>
									</div>
								`
							}).Show();
						},
						clss: 'pk_inact',
						setup: function (obj) {
							obj.setAttribute('data-id', 'upload');
							
							// Enable button if we have a user ID
							if (window.currentUserId) {
								obj.classList.remove('pk_inact');
							}
	...
	function _makeUIToolbar (UI) {
		...
		UI.listenFor ('DidAudioProcess', function( val ) {
			...
			if (time > -1)
			{
				...
				// Check if the engine and wavesurfer exist before using them
				if (PKAudioEditor && PKAudioEditor.engine && PKAudioEditor.engine.wavesurfer && 
					PKAudioEditor.engine.wavesurfer.ZoomFactor > 1)
				{
					...
					if (wvpnt) wvpnt.style.left = ((perc * 10000)>>0)/100 + '%';
				}
			}
...
function showSaveDialog(audioData, format, mimeType) {
  new PKSimpleModal({
    title: 'Save Track',
    buttons: [
      {
        title: 'Save',
        callback: function(q) {
          const title = q.el_body.querySelector('#track-title').value;
          const artist = q.el_body.querySelector('#track-artist').value.split(',').map(a => a.trim());
          const genre = q.el_body.querySelector('#track-genre').value.split(',').map(g => g.trim());
          const projectId = q.el_body.querySelector('#project-id').value;
          const imageFile = q.el_body.querySelector('#cover-image').files[0];
    ...
    body: `
      <div class="pk_row">
        ...
        <input type="text" id="track-title" class="pk_txt" value="${currentTrackName || ''}" />
      </div>
      <div class="pk_row">
        <label for="track-artist">Artist(s)</label>
        <input type="text" id="track-artist" class="pk_txt" placeholder="Separate multiple artists with commas" />
      </div>
      <div class="pk_row">
        <label for="track-genre">Genre(s)</label>
        <input type="text" id="track-genre" class="pk_txt" placeholder="Separate multiple genres with commas" />
      </div>
      <div class="pk_row">
        <label for="project-id">Project ID</label>
        <input type="text" id="project-id" class="pk_txt" />
      </div>
      <div class="pk_row">
...
// Only keep the getCurrentUserId function and error handler
...
// At the end of the initialization code in ui.js
window.addEventListener('load', function() {
  // Signal that the editor is ready to receive messages
